...
impl FeeOracle for BasicFeeOracle {
    fn suggest(&self) -> FeePlan {
        ...
        FeePlan {
            ...
            cu_price_microlamports: self.min_cu_price,
        }
    }
    fn clamp(&self, mut p: FeePlan) -> FeePlan {
        if p.cu_limit > self.max_cu_limit {
            p.cu_limit = self.max_cu_limit;
        }
        if p.cu_price_microlamports < self.min_cu_price {
            p.cu_price_microlamports = self.min_cu_price;
        }
        if p.cu_price_microlamports > self.max_cu_price {
            p.cu_price_microlamports = self.max_cu_price;
        }
        p
    }
}
...
mod tests {
    ...
    fn clamp_enforces_bounds() {
        ...
        let p = o.clamp(FeePlan {
            cu_limit: o.max_cu_limit + 1_000,
            cu_price_microlamports: o.max_cu_price + 1,
        });
        assert_eq!(p.cu_limit, o.max_cu_limit);
        assert_eq!(p.cu_price_microlamports, o.max_cu_price);

        let p2 = o.clamp(FeePlan {
            cu_limit: 0,
