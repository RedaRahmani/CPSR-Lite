# Confidential Planner (Arcium MPC Integration)

This program provides encrypted, privacy-preserving transaction planning using Arcium's secure Multi-Party Computation (MPC) network.

## Overview

The confidential planner takes a set of user intents (transactions) as encrypted input, computes a conflict-free execution plan inside a secure MPC enclave, and returns encrypted chunk assignments. The planner logic runs entirely off-chain in the Arcium network, ensuring that:

- Intent details remain encrypted end-to-end
- No single party can observe the planning logic or intermediate states
- Outputs are verifiable on-chain via cryptographic commitments

---

# Original Arcium Project Structure
