...
impl Chunk {
    ...
    pub fn id_bound(&self) -> Hash32 {
        let sess = self.session_authority.map(|k| k.to_bytes()).unwrap_or([0u8; 32]);
        let nonce_bytes = self.nonce.to_le_bytes();
        dhash(b"CPSR:CHUNKBOUND", &[&self.merkle_root, &self.occ_root(), &sess, &nonce_bytes])
    }
...
impl Bundle {
    ...
    pub fn id(&self) -> Hash32 { self.bundle_root }
}
...
mod tests {
    ...
    fn chunk_id_stable_under_occ_permutation() {
        ...
        let c1 = Chunk {
            intents: vec![],
            occ_versions: vec![
                AccountVersion { key: Pubkey::new_unique(), lamports: 1, owner: Pubkey::new_unique(), data_hash64: 10, slot: 5 },
                AccountVersion { key: Pubkey::new_unique(), lamports: 2, owner: Pubkey::new_unique(), data_hash64: 20, slot: 7 },
            ],
            merkle_root: [1u8; 32],
...
mod more_tests {
    ...
    fn mk_intent(program: Pubkey, keys: &[Pubkey], data: Vec<u8>) -> UserIntent {
        let metas: Vec<AccountMeta> = keys.iter().map(|k| AccountMeta::new_readonly(*k, false)).collect();
    ...
    fn intent_and_occ_roots_and_ids() {
        ...
        assert_ne!(bound, id1, "binding to session/nonce should change id_bound()");
    }
...
fn bundle_root_changes_with_chunk_order() {
    ...
    use solana_program::pubkey::Pubkey;
    use crate::hash::ZERO32; // if you export it; otherwise just use [0u8; 32]
